...
Procedimento executado: alterar temporariamente uma regra (ex.: em `PoliticaDesconto`, trocar `subtotal >= 50_000` por `subtotal >= 40_000`), rodar `mvn clean test` e confirmar que `comumComSubtotalAbaixoDoLimiteNaoRecebeDesconto` (que usa `subtotal = 49_999`) passa a falhar, evidenciando que o teste detecta a mutação; em seguida desfazer a alteração e confirmar que a suíte volta a passar por completo antes da entrega.
